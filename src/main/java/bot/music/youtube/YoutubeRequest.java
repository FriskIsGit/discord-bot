...
public class YoutubeRequest{
    private volatile static boolean hasActiveRequest;
    private volatile static long lastRequest = 0;
    ...
    public void process(){
        if(requestText.length() < MIN_REQUEST_LENGTH){
            actions.sendAsMessageBlock(requestChannel, "Invalid request");
            return;
        }

        if(System.currentTimeMillis()-lastRequest < REQUEST_RATE_LIMIT_MS){
            actions.sendAsMessageBlock(requestChannel, "Yer bweein rate limited uwu nya ;3 0.0");
            return;
        }
        ...
        if(result.type == StreamType.NONE){
            actions.messageChannel(requestChannel,"Invalid request");
            return;
        }
        YoutubeVideoInfo ytVideoInfo = youtube.getVideoInformation(result.videoId);
        if(ytVideoInfo == null){
            actions.messageChannel(requestChannel,"Unable to get info about video");
            return;
        }
    ...
    private void displayVideoInformation(){
        actions.sendAsMessageBlock(requestChannel, ytVideoInfo.detailsToString());
        actions.sendAsMessageBlock(requestChannel, ytVideoInfo.getAvailableVideoFormats());
        actions.sendAsMessageBlock(requestChannel, ytVideoInfo.getAvailableAudioFormats());
        actions.sendAsMessageBlock(requestChannel, ytVideoInfo.getAvailableVideoWithAudioFormats());
        actions.sendAsMessageBlock(requestChannel, "Alleged best formats: \n" + ytVideoInfo.bestFormatsToString());
    }
    ...
    private void downloadVideo(int formatNumber){
        ...
        actions.sendFile(requestChannel, video);
    }
    ...
    private void downloadVideoWithAudio(int formatNumber){
        ...
        actions.sendFile(requestChannel, videoWithAudio);
    }
    ...
    public void downloadAudioImpl(AudioFormat format){
        ...
        System.out.println("Time taken on download: " + (en-st));
        actions.sendFile(requestChannel,audioFile);
    }
