...
public class LyricsCommand extends Command {
    ...
    protected void executeImpl(String commandName, MessageReceivedEvent message, String... args) {
        ...
        if (url.startsWith(GENIUS_URL)) {
            GetRequest webpageRequest = Unirest.get(url);
            webpageRequest.header("Accept", "text/html");
            webpageRequest.header("Accept-Language", "en-US;q=0.7");
            webpageRequest.header("User-Agent", "Mozilla/5.0 Gecko/20100101");
            HttpResponse<String> pageResponse = webpageRequest.asString();

            if (pageResponse.getStatus() != 200) {
                //exception
                return;
            }
            String lyrics = Scrapper.scrapeGeniusLyrics(pageResponse.getBody());
        ...
        GetRequest request = Unirest.get(GENIUS_API_URL + "/search?q=" + encode(song))
                .header("Content-Type", "application/json")
                .header("Authorization", "Bearer " + Bot.getConfig().geniusToken);
        ...
        GetRequest webpageRequest = Unirest.get(mostAccurate.lyricsURL);
        webpageRequest.header("Accept", "text/html");
        webpageRequest.header("Accept-Language", "en-US;q=0.7");
        webpageRequest.header("User-Agent", "Mozilla/5.0 Gecko/20100101");
        HttpResponse<String> pageResponse = webpageRequest.asString();
