...
public class AICommand extends Command {
    ...
    protected void executeImpl(String commandName, MessageReceivedEvent message, String... args) {
        ...
        if (commandName.equals("ai21")) {
            ...
            if (isTask) {
                ...
                HttpRequestWithBody request = Unirest.post(task.getEndpoint())
                        .header("Authorization", "Bearer " + Bot.getConfig().ai21Token)
                        .header("Accept", "application/json")
                        .header("Content-Type", "application/json")
                        .connectTimeout(TIMEOUT_MS)
                        .socketTimeout(TIMEOUT_MS);
                JsonBody payload = adaptJsonBodyToTask(task);
                request.body(payload.getBytes());
                ...
                try {
                    ...
                    embed = createInfoEmbed("Error", "Request likely timed out");
                    actions.sendEmbed(channel, embed);
                    return;
                }

                if (response.getStatus() != 200) {
                    embed = createInfoEmbed(response.getStatus(), response.getBody());
                    actions.sendEmbed(channel, embed);
                    return;
                }

                String content = extractRelevantContentFromBody(task, response.getBody());
                embed = createInfoEmbed(task.toString(), content);
                actions.sendEmbed(channel, embed);
            } else {
                ...
                HttpRequestWithBody request = Unirest.post(url)
                        ...
                        .connectTimeout(TIMEOUT_MS)
                        .socketTimeout(TIMEOUT_MS);
                JsonBody payload = JsonBody.body()
    ...
    private String extractRelevantContentFromBody(AITask task, String body) {
    ...
    private static MessageEmbed createInfoEmbed(int title, String desc) {
        return createInfoEmbed(String.valueOf(title), desc);
    }
    ...
    private List<Model> retrieveOpenAIModels() {
        GetRequest request = Unirest.get(OPENAI_MODELS_URL)
                .header("Content-Type", "application/json")
